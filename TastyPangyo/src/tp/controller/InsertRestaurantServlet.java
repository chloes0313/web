...
public class InsertRestaurantServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		ArrayList<String> imageName = new ArrayList<>(); // 업로드된 파일명 저장할 List변수
	...
	for(FileItem item : list){
		...
		String reqName = item.getFieldName(); // 요청파라미터의 이름 조회
			...
			if(item.getSize() != 0){
				...
				vo.setImages(imageName);
				
				session.setAttribute("result", imageName);
			...
			// 응답 처리
			resp.sendRedirect("/TastyPangyo/restaurant/regist_success.jsp");
