...
public class SelectRestaurantServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String resSelectKey = req.getParameter(paramName); // 파라미터 name값으로 가져온
															// value를 조회값으로
															// 사용한다.
		...
		switch (paramName) {
		case "allRes":
			if (member == null || !member.getMemberId().equals("admin") || req.getAttribute("common").equals("true")) { // 비회원
																			// 또는
																			// 일반회원이면
																			// 조회만
																			// 가능.
				session.setAttribute("resList", service.selectAllRestaurant());
				req.getRequestDispatcher("/restaurant/common_Allselect.jsp").forward(req, resp);
				break;
			}
			session.setAttribute("resList", service.selectAllRestaurant());
			req.getRequestDispatcher("/restaurant/manager_Allselect.jsp").forward(req, resp);
			break;

		case "location":
			if (member == null || !member.getMemberId().equals("admin") || req.getAttribute("common").equals("true")) {
