...
public class RestaurantServiceImpl implements RestaurantService {
   ...
   int count =0;
   List<Restaurant> list = new ArrayList<Restaurant>();// 테스트용 삭제요망
   ...
   ReviewDaoImpl rdao = ReviewDaoImpl.getInstance();
...
//         System.out.println(count + "행 증가");
   ...
   public List<Restaurant> selectAllRestaurant() {
      ...
      try{
      session = factory.openSession();
      List<Restaurant> list = dao.selectAllRestaurant(session);
      for(int i = 0; i < list.size(); i++){
    	  try{
    	  list.get(i).setAvgKostar(rdao.selectAverageOfKostarByRestaurantId(session, list.get(i).getRestaurantId()));
    	  }catch(Exception e){
    		  list.get(i).setAvgKostar(0);
    	  }
      }
      return list;
   ...
   public List<Restaurant> selectAllRestaurantByHit() {
      ...
      try{
      session = factory.openSession();
      list = dao.selectAllRestaurantByHit(session);
      
     
      for(int i = 0; i < list.size(); i++){
    	  list.get(i).setImages((ArrayList<String>)sdao.selectRestaurantImageById(list.get(i).getRestaurantId()));
      }
     return  list;
      }finally{
         session.commit();
   ...
   public List<Restaurant> selectRestaurantIdByAvgKostar() {
      ...
      try{
      session = factory.openSession();
      List<Restaurant> list = dao.selectRestaurantIdByAvgKostar(session);
      for(int i = 0; i < list.size(); i++){
         list.get(i).setAvgKostar(rdao.selectAverageOfKostarByRestaurantId(session, list.get(i).getRestaurantId()));
         list.get(i).setImages((ArrayList<String>)sdao.selectRestaurantImageById(list.get(i).getRestaurantId()));
      }
